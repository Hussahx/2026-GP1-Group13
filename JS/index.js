...
if (!rFile || !rFile.files.length) {
  ...
  return 'متوسط';
          ...
          AdminID:             '',
          CurrentVolunteers:   0,
          // مهم: نُنشئ الحقل من البداية بمصفوفة فاضية. البلاغات القديمة
          // كانت تحتوي هذا الحقل أصلاً، لكن البلاغات الجديدة كانت تُنشأ
          // بدونه تماماً — وهذا على الأغلب هو سبب فشل عملية "تولّي/انضمام"
          // بصمت على البلاغات الجديدة (لو صلاحيات Firestore تشير لهذا
          // الحقل، الإشارة لحقل غير موجود إطلاقاً تُفشل الشرط بالكامل).
          teamMembers:            [],
